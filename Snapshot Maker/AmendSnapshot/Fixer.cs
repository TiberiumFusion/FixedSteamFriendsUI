...
namespace TiberiumFusion.FixedSteamFriendsUI.SnapshotMaker.AmendSnapshot
{
    public static class Fixer
    {
        ...
        public static void ModifySteamchatDotComSnapshot(string snapshotDirectoryPath)
        {
            // --------------------------------------------------
            ...
            // We need to make the @font-face url()s relative to the snapshot structure
            if (Scraper.ResourceTypesToSave[Scraper.ResourceCategory.CssFonts]) // but only if we actually downloaded the fonts for the snapshot
            {
        ...
        private static void WriteModifiedFileUtf8(string path, string contents)
        {
